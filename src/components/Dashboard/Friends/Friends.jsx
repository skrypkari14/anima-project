    ...
    return (
        <div className='p-6'>
            {isOpen && (
                <div className='fixed z-50 px-2 sm:px-0 w-screen h-screen bg-primary/40 left-0 top-0 flex justify-center items-center'>
                    <div className='bg-white max-w-[460px] w-full py-6 flex flex-col justify-center rounded-md'>
                        <h4 className='px-6 text-lg font-semibold text-primary'>Добавить друга</h4>
                        <div className='p-6 border-b border-subtle'>
                            <label>Введите никнейм друга</label>
            ...
            <div className='flex flex-col justify-center items-center text-center gap-4 md:flex-row md:text-left md:justify-between md:items-end'>
                <div>
                    <h1 className='text-2xl font-semibold text-primary'>Друзья</h1>
            ...
            <div className='mt-6 bg-white rounded-md flex flex-col'>
                {friends.map((friend, index) => (
                    <div
                        key={friend.username}
                        className={`p-6 flex flex-col sm:flex-row justify-between items-center ${index + 1 !== friends.length ? 'border-b border-subtle' : ''}`}
                    >
                        <div className='flex items-center gap-4'>
                            <Link to={`/dashboard/user/${friend.username}`} className='relative'>
                                ...
                                />
                                {friend.online && (
                                    <div className='w-3.5 h-3.5 rounded-full border-2 border-white bg-[#26BD6C] absolute bottom-0 right-0' />
                                )}
                            </Link>
                            <div>
                                <h4 className='text-primary font-medium'>{friend.username}</h4>
